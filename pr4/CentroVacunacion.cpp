...
bool CentroVacunacion::administrarDosis( Usuario *u, Dosis::fabricante fabricante ){
    ...
    if( fabricante == Dosis::null )
        ...
        std::list<Usuario*>::iterator busca = _seVacunaAqui.begin();
        std::list<Usuario*>::iterator borradoUsuario;
        while( busca != _seVacunaAqui.end() ){
            if( (*busca)->GetNSS() == u->GetNSS() ){
                comprobar = true;
                borradoUsuario = busca;
            }
            ++busca;
        }
        ...
        for( it = _almacena.begin(); it != _almacena.end(); it++ ){
            ...
            if( (*it).second.getEstado() == Dosis::enAlmacen ){ //¿Almacén vacío?
                ...
                if( fabricante == (*it).second.getFabricante() ){ //==Fabricante
                    ...
                    _seVacunaAqui.erase(borradoUsuario);
                    return true;
                }  
            }
        }
